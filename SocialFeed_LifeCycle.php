...
include_once('SocialFeed_InstallIndicator.php');

class SocialFeed_LifeCycle extends SocialFeed_InstallIndicator {
	...
	public function adminDataPage(){
	    ...
	    $list = new AdminList();
	    $list->setTitle("Data");
	    $list->setTable($this->prefixTableName("data"));
	    $list->setSortDir("desc");
	    $list->setSortColumn("submission_date");

	    $list->addAction("show");
		if($_REQUEST["action"] == "show"){
			$wpdb->query($wpdb->prepare("UPDATE ".$this->prefixTableName("data")." SET visible = 't' WHERE id = %d", $_REQUEST["id"]));
		}

	    $list->addAction("hide");
		if($_REQUEST["action"] == "hide"){
			$wpdb->query($wpdb->prepare("UPDATE ".$this->prefixTableName("data")." SET visible = 'f' WHERE id = %d", $_REQUEST["id"]));
		}
	    ...
	    $list->setQuery("
	    				SELECT d.id,
							   ...
							   d.url,
							   d.content,
	    					   IF(d.visible = 't', 'Yes', 'No') visible
	    				  FROM ".$this->prefixTableName("data")." d
	    				");

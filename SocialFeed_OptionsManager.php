...
class SocialFeed_OptionsManager {
    ...
    public function settingsPage() {
        if (!current_user_can('manage_options')) {
            wp_die(__('You do not have sufficient permissions to access this page.', 'socialfeed'));
        }
        ...
        ?>
        <div class="wrap">
            <h2><?php _e('System Settings', 'socialfeed'); ?></h2>
            <table class="form-table"><tbody>
            <tr><td><?php _e('System', 'socialfeed'); ?></td><td><?php echo php_uname(); ?></td></tr>
            <tr><td><?php _e('PHP Version', 'socialfeed'); ?></td>
                <td><?php echo phpversion(); ?>
                <?php
                if (version_compare('5.2', phpversion()) > 0) {
                    echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
                    _e('(WARNING: This plugin may not work properly with versions earlier than PHP 5.2)', 'socialfeed');
                    echo '</span>';
                }
                ?>
                </td>
            </tr>
            <tr><td><?php _e('MySQL Version', 'socialfeed'); ?></td>
                <td><?php echo $this->getMySqlVersion() ?>
                    <?php
                    echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
                    if (version_compare('5.0', $this->getMySqlVersion()) > 0) {
                        _e('(WARNING: This plugin may not work properly with versions earlier than MySQL 5.0)', 'socialfeed');
                    }
                    echo '</span>';
                    ?>
                </td>
        ...
        */
        ...
        ?>
            ...
            <h2><?php echo $this->getPluginDisplayName(); echo ' '; _e('Settings', 'socialfeed'); ?></h2>
            ...
            <?php settings_fields($settingsGroup); ?>
                ...
                <p class="submit">
                    <input type="submit" class="button-primary"
                           value="<?php _e('Save Changes', 'socialfeed') ?>"/>
                </p>
            </form>

            <h2><?php echo ' Fetch data';  ?></h2>
			
			<p>Use this url <strong><a href="<?= $this->getAjaxUrl("SocialFeedFetch"); ?>" target="_blank"><?= $this->getAjaxUrl("SocialFeedFetch"); ?></a></strong> to fetch the data. </p>

			<p>Use socialfeed_get function to load posts:</p>
			<pre>
$rows =  socialfeed_get(
	array("feeds" =>
		array(
    ...
    protected function getOptionValueI18nString($optionValue) {
        switch ($optionValue) {
            case 'true':
                return __('true', 'socialfeed');
            case 'false':
                return __('false', 'socialfeed');

            case 'Administrator':
                return __('Administrator', 'socialfeed');
            case 'Editor':
                return __('Editor', 'socialfeed');
            case 'Author':
                return __('Author', 'socialfeed');
            case 'Contributor':
                return __('Contributor', 'socialfeed');
            case 'Subscriber':
                return __('Subscriber', 'socialfeed');
            case 'Anyone':
                return __('Anyone', 'socialfeed');
        }
        return $optionValue;
    }
